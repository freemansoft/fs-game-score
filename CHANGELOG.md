...
- Added two trick-taking game modes with **calculated bid/tricks scoring**: **Oh Hell** (exact bid → 10 + bid, miss → 0) and **Wizard** (exact → 20 + 10×bid, miss → −10 per trick over/under). A bid/tricks round editor derives the round score; highest total wins. The round editor shows a note about a known limitation: a made 0-bid (bid 0, 0 tricks) currently scores 0. Localized in English, Spanish, and French. See [docs/Game-Modes-Roadmap.md](docs/Game-Modes-Roadmap.md) (Tier 1A)
