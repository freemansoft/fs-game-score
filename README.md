...
3. This project uses `fvm` to set the correct flutter version. Install `fvm`.
   ...
   1. Use in an elevated command powershell prompt to install `fvm`. Needs to be done only once
      1. `choco install fvm`
      1. `choco upgrade fvm`
   1. From a command prompt in vscode after installing `fvm`
...
The integration tests were AI-generated and are AI-maintained. You can run them with
...
**Claude Code is the only supported AI coding tool for this repository.** Its configuration lives in two places:

- **[CLAUDE.md](CLAUDE.md)** — project rules: architecture, Riverpod patterns, testing conventions, widget-key rules
- **[`.claude/skills/`](.claude/skills/)** — project skills (`fs-game-score-*`, `run-fs-game-score`), auto-discovered when each skill has a valid `SKILL.md` with YAML frontmatter
- **[`.claude/settings.json`](.claude/settings.json)** — plugin marketplaces and enabled plugins

Upstream Dart/Flutter skills are not checked in. They come from the `dart-flutter` plugin published by the Flutter team at [flutter/agent-plugins](https://github.com/flutter/agent-plugins), which also wires up the Dart MCP server.

#### Dart/Flutter skills plugin

Claude Code picks the plugin up automatically from [`.claude/settings.json`](.claude/settings.json) (`extraKnownMarketplaces` + `enabledPlugins`). To install it by hand:
...
This workspace enables superpowers via [`.claude/settings.json`](.claude/settings.json) → `enabledPlugins`.
