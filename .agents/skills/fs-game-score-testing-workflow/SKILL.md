...
- Semantic labels are screen-reader text and **must be localized** — canonical rule in **`fs-game-score-flutter-patterns` → Localization**. A CI guard (`tool/check_semantic_l10n.py`) fails the build on hardcoded labels.
