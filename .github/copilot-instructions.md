...
* **Riverpod:** If a dependency injection solution beyond manual constructor
  injection is explicitly requested, `riverpod` can be used to make services,
...
* **Screen Reader Testing:** Regularly test your app with TalkBack (Android) and
  VoiceOver (iOS).
* **Semantic Label Keys** Use the ValueKey class to define semantic label keys for
  widgets that will be read by screen readers. The ValueKey should have a descriptive
  string that clearly identifies the widget's purpose. The ValueKey should be assigned to a variable instead of being used inline to improve readability and maintainability and make it easier to update the key in the future if needed. Tests should use these keys to verify accessibility features.
