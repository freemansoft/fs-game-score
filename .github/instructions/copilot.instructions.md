...
* **Navigation:** Use a modern routing package like `auto_route` or `go_router`.
  ...
  `go_router`.
* **Widget Keys** Use the ValueKey class to define widget keys for
  widgets that are important for testing, such as buttons, and all form fields. The ValueKey should be short. The ValueKey should be generated by a function instead of being created inline to improve readability and maintainability and make it easier to update the key in the future if needed. Tests should use these key functions to locate widgets.
